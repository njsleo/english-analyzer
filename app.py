...
# ==========================================
    ...
    if IS_ADMIN:
        with st.expander("👑 馆长专属：上传新教材/小说", expanded=False):
            ...
            lib_cat = st.selectbox("选择分类", base_categories[1:]) # 老板上传时选择分类
                    ...
                    if filtered_lib:
                        ...
                        with col_tools:
                            ...
                            with tab_dict:
                                ...
                                if st.button("💡 翻译并存库", type="primary", use_container_width=True, key=f"btn_w_{i}"):
                                    if lookup_word:
                                        with st.spinner("查词中..."):
                            ...
                            with tab_clip:
                                ...
                                if st.button("✍️ 解析并归档", type="primary", use_container_width=True, key=f"btn_c_{i}"):
                                    if clip_sentence:
                                        with st.spinner("解析中..."):
...
# ==========================================
    ...
    if 'analysis_result' in st.session_state:
        ...
        for i, s in enumerate(res.get('sentences', [])):
            st.markdown(f"""<div style='background:#F4F6ED; border-radius:8px; padding:12px; margin-bottom:8px; border:1px solid #DFE3DB;'>
